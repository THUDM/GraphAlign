    ...
    def __init__(self,
        ...
        if num_layers == 1:
            self.gat_layers.append(GATConv(
                ...
                allow_zero_in_degree=allow_zero_degree, top_k=top_k, hhsize_time=hhsize_time, num_expert=num_expert, moe=moe if 0 in moe_layer else False , moe_use_linear=moe_use_linear))
        else:
            # input projection (no residual)
            self.gat_layers.append(GATConv(
                in_dim, num_hidden, nhead,
                feat_drop, attn_drop, negative_slope, residual, create_activation(activation), norm=norm,
                concat_out=concat_out, allow_zero_in_degree=allow_zero_degree, top_k=top_k, hhsize_time=hhsize_time, num_expert=num_expert,  moe=moe if 0 in moe_layer else False , moe_use_linear=moe_use_linear))
            # hidden layers
            for l in range(1, num_layers - 1):
                ...
                self.gat_layers.append(GATConv(
                    ...
                    feat_drop, attn_drop, negative_slope, residual, create_activation(activation), norm=norm,
                    concat_out=concat_out, allow_zero_in_degree=allow_zero_degree, top_k=top_k, hhsize_time=hhsize_time, num_expert=num_expert,  moe=moe if l in moe_layer else False , moe_use_linear=moe_use_linear))
            # output projection
            self.gat_layers.append(GATConv(
                ...
                concat_out=concat_out, allow_zero_in_degree=allow_zero_degree, top_k=top_k, hhsize_time=hhsize_time, num_expert=num_expert, moe=moe if num_layers-1 in moe_layer else False , moe_use_linear=moe_use_linear))
